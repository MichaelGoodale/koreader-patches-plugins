...
local STACK_DECOR_PATH = DataStorage:getDataDir() .. "/patches/cat.png"
...
local SETTINGS = {
    SHOW_BACKGROUND = "bookshelf_show_background",
    SHOW_STACK_DECOR = "bookshelf_show_stack_decor",
    SHOW_TIME_LEFT = "bookshelf_show_time_left",
    SHOW_PERCENT = "bookshelf_show_percent",
    SHOW_BANDS = "bookshelf_show_bands",
    USE_RANDOM_COLORS = "bookshelf_use_random_colors",
    NUM_BOOKS = "bookshelf_num_books",
    FINISHED_THRESHOLD = "bookshelf_finished_threshold",
}
...
local function getBookColor(index, use_random)
    if isColorScreen() then
        local colors = {
            Blitbuffer.ColorRGB32(120, 190, 120, 255),
            Blitbuffer.ColorRGB32(130, 160, 220, 255),
            Blitbuffer.ColorRGB32(190, 140, 210, 255),
            Blitbuffer.ColorRGB32(220, 120, 120, 255),
        }
        if use_random then
...
local function getBandColor()
    if isColorScreen() then
        return Blitbuffer.ColorRGB32(230, 190, 50, 255)
    else
...
local function buildBookshelfWidget()
    ...
    for i = 1, book_count do
        local book = books[i]
        local left_offset = Screen:scaleBySize(math.random(0, 20))
        ...
        local width_percent = 0.60 + (width_factor * 0.25)
        ...
        if i < book_count then
            table.insert(books_stack, VerticalSpan:new { width = spacing })
    ...
    local stack_decor_size = Screen:scaleBySize(200)
    ...
    if show_stack_decor and stack_decor_widget then
        table.insert(final_widget, VerticalGroup:new {
            VerticalSpan:new {
                width = top_margin - stack_decor_size + Screen:scaleBySize(10),
            },
            HorizontalGroup:new {
                HorizontalSpan:new { width = top_width - stack_decor_size + Screen:scaleBySize(20) },
                stack_decor_widget,
            },
        })
    end
        ...
        if result and result[1] and result[1].sub_item_table then
            ...
            table.insert(wallpaper_submenu, {
                ...
                sub_item_table = {
                    ...
                    createSpinnerMenuItem(_("Set number of books"), SETTINGS.NUM_BOOKS, DEFAULTS.NUM_BOOKS, 1, 10, 1),
                    createSpinnerMenuItem(_("Set 'finished' threshold (%)"), SETTINGS.FINISHED_THRESHOLD,
                        DEFAULTS.FINISHED_THRESHOLD, 80, 100, 1),
